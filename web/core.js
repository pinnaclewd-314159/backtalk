 ...
 */
/* ============================================================
   Adapted for the browser PTT client from ai-visualizer/core.js.
   See docs/superpowers/specs/2026-09-10-web-ptt-client-design.md.

   Differences from the original:
     - State comes from AV.setRaw(obj), pushed by ptt.html's WebSocket
       handler, instead of polling /state -- a remote page has no local
       bus files to read, and a persistent connection already exists
       here anyway (lower latency than polling).
     - The burn-in idle-dim guard, transcript panel, and rate-limit HUD
       are dropped entirely -- all three read local files (/log,
       /rate_limit) or exist for the always-on PC kiosk display
       specifically; nothing in this server serves those endpoints,
       and polling them forever would just be silent dead weight.
     - Demo mode, shot mode, the mic-level helpers, the thinking-sound
       player, and the render utility belt are otherwise UNCHANGED from
       the original -- kept for parity and because trimming them adds
       risk without removing anything actually broken here.

   A face includes this script, calls AV.init(opts), then reads these
...
const AV = (() => {
  ...
  /* ------------------------------ state push -------------------------------
     Replaces the original's /state polling: ptt.html's WebSocket handler
     calls AV.setRaw({state: "..."}) whenever a state message arrives. */
  ...
  function tick(dt) {
    ...
    const s = raw.samples;
    A.rawSamples = s && s.length ? s : null;
    if (s && s.length) {
      let mx = 0;
  ...
  A.init = (opts = {}) => {
    ...
    if (DEMO) {
      ...
      applyConfig({});   // no /config endpoint on this server -- use defaults
    }
    return A;
  };
