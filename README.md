...
- **How long a pause it takes to end your turn (hands-free only):** `silence_ms` in `backtalk.json`, 480 by default. If pauses mid-thought keep getting cut off and sent early, raise it (900–1500 is a reasonable range); lower it if turns feel sluggish on a quiet mic. Not a spoken voice-console setting — hand-edit the file and restart the voice line.
